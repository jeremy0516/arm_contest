...
/*oid SystemClock_Config_Interrupt(int speed, int load){
...
void GPIO_init_AF(){
	...
	//GPIOA->AFR[15] &= ~GPIO_AFRH_AFRH7_Msk;
	//GPIOA->AFR[15] |= (1 << GPIO_AFRH_AFRH7_Pos);
}
...
void GPIO_init_AF2(){
	...
	GPIOF->MODER |= (2 << GPIO_MODER_MODER9_Pos);
	// Set AFRL
	GPIOF->AFR[0] &= ~GPIO_AFRH_AFRH2_Msk;
	GPIOF->AFR[0] |= (1 << GPIO_AFRH_AFRH2_Pos);
}
